...
namespace MVS::ARKIT {
    ...
    static void buildMask(const std::string& mask_name, Image& image, const cv::Size& depthMapSize) {

        IMAGEPTR pImage = Image::OpenImage(mask_name);

        if(pImage == NULL || !Image::ReadImage(pImage, image.mask)) {
            throw std::runtime_error("Failed to open mask image: " + mask_name);
        }

        unsigned resolution = std::max(depthMapSize.width, depthMapSize.height);

        const REAL scale = (REAL)resolution /MAXF(image.width, image.height);

        const cv::Size scaledSize(Image8U::computeResize(image.GetSize(), scale));
        
        cv::resize(image.mask, image.mask, scaledSize, 0, 0, cv::INTER_NEAREST);
    }
    ...
    static void buildImage(const ARKITFrame& frame, Image& image, const cv::Size& depthMapSize) {
        ...
        if(frame.mask_name != "") {
            buildMask(frame.mask_name, image, depthMapSize);
        }
    ...
    Camera ARKITScene::loadCamera(const ARKITFrame& frame, const cv::Size& newSize, bool transToRef, int refCamera) {
        cv::Size initSize = newSize.empty() ? depthMapSize : newSize;

        ASSERT(!aspectRatioDiff(depthMapSize, initSize));

        // referenct camera
        const Camera& cameraRef = scene->images[refCamera].camera;
        ...
        if(transToRef) {
            ...
            // R_2 * (C_1 - C_2)
            auto C = camera.R * (cameraRef.C - camera.C);
    ...
    void ARKITScene::buildCoarsePointcloud(const std::string& ply_path, float max_depth) {
        ...
        for(const ARKITFrame& frame: arkitFrames) {
            ...
            // load camera for depthmap projection
            const Camera& camera = loadCamera(frame, depthMapSize, false);
            ...
            for(int row = 0; row < deptmap.rows; row++) {
                for(int col = 0; col < deptmap.cols; col++) {
                    ...
                    if(image.mask.empty() || image.mask.at<uint8_t>(row, col) == 0) {
                        continue;
                    }

                    float depth = deptmap.at<float>(row, col);
                    if(std::isnan(depth) || depth < 1e-6 || depth >= max_depth) {
                        continue;
                    }
